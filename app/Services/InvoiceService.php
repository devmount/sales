...
class InvoiceService
{
    ...
    public static function generatePdf(Invoice $invoice): string {
        ...
        // Table total
        $pdf->setFillColor(Color::MAIN->pdf())
            ->rect(0, 165, 210, 50, PdfRectangleStyle::FILL)
            ->setDrawColor(Color::LINE3->pdf())
            ->setLineWidth(0.3);
        if ($invoice->discount) {
            // Total with discount
            $pdf->line(124, 198, 194, 198)
                ->setTextColor(Color::TEXT->pdf())
                ->setFont('FiraSans-ExtraLight')
        ...
        $filename = strtolower("{$data['number']}_{$label['invoice']}_{$conf['company']}");

        // Generate and add XML attachment
        $xmlFile = self::generateEn16931Xml($invoice);
        // FIXME: filename not shown
        $pdf->addAttachment(Storage::path($xmlFile), "$filename.xml")
            ->setPageMode(PdfPageMode::USE_ATTACHMENTS);

        // Save document
        $pdf->output(PdfDestination::FILE, Storage::path("$filename.pdf"));
        return "$filename.pdf";
    }
