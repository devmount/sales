...
document.addEventListener('DOMContentLoaded', () => {
    ...
    doc.setTextColor(colors.light).setFont('FiraSansExtraLight')
        .setFontSize(26).text('{{ trans_choice("invoice", 1) }}'.toUpperCase(), 105, 27, { align: 'center' })
        .setFontSize(9)
    ...
    doc.setTextColor(colors.gray).setFont('FiraSansExtraLight').setFontSize(8)
        ...
        .setTextColor(colors.gray)
            .setFontSize(10)
                ...
                .text('{{ __("invoiceNumber") }}', 144, 62.8)
                .text('{{ __("invoiceDate") }}', 144, 68.8)
    ...
    doc.setLineWidth(0.8)
        ...
        .setFontSize(8)
            .setFont('FiraSansExtraLight').setTextColor(colors.dark)
                .text('{{ __("statementOfWork") }}', 15, 124)
                .text(billedPerProject ? '{{ __("flatRate") }}' : '{{ __("inHours") }}', 115, 124, { align: 'center' })
                .text(billedPerProject ? '{{ __("flatRate") }}' : '{{ __("perHour") }}', 146, 124, { align: 'center' })
            .setTextColor(colors.light)
                .text(billedPerProject ? '{{ __("sum") }}' : '{{ __("sumOfAllPositions") }}', 182, 124, { align: 'center' })
        ...
        doc.setFillColor(colors.main).rect(0, 165, 210, 50, 'F').setDrawColor(colors.line3).setLineWidth(0.3).line(124, 196, 194, 196)
            .setTextColor(colors.text)
                .setFont('FiraSansExtraLight').setFontSize(13)
                    .text('{{ __("amountNet") }}', 160, 181, { align: 'right' })
            ...
            .setTextColor(colors.light)
                .setFont('FiraSansRegular').setFontSize(16)
                    .text('{{ __("totalAmount") }}', 160, 205, { align: 'right' })
    ...
    // invoice table total with discount
        ...
        doc.setFillColor(colors.main).rect(0, 165, 210, 50, 'F').setDrawColor(colors.line3).setLineWidth(0.3).line(124, 198, 194, 198)
            .setTextColor(colors.text)
                .setFont('FiraSansExtraLight').setFontSize(13)
                    .text('{{ __("amountNet") }}', 160, 177, { align: 'right' })
            ...
            .setTextColor(colors.light)
                .setFont('FiraSansRegular').setFontSize(16)
                    .text('{{ __("totalAmount") }}', 160, 207, { align: 'right' })
    ...
    doc.setDrawColor(colors.line).setLineWidth(0.4).line(10, 272, 202, 272)
        ...
        .setLineHeightFactor(1.3).setFontSize(9).setTextColor(colors.gray)
            ...
            .text(['{{ __("iban") }}', '{{ __("bic") }}', '{{ __("bank") }}', '{{ __("holder") }}'], 90, 277, { align: 'right' })
            .text(['{{ __("vatId") }}', '{{ __("taxOffice") }}'], 170, 277, { align: 'right' })
    ...
    paginatedPositions(positions).forEach(positions => {
        ...
        doc.addImage(config.logo, 'JPEG', 12, 13, 22, 22);
        doc.setTextColor(colors.light).setFont('FiraSansExtraLight')
            .setFontSize(26).text('{{ __("deliverables") }}'.toUpperCase(), 105, 27, { align: 'center' })
            .setFontSize(9)
                .text(config.email, 202, 25, { align: 'right' })
                .text(config.phone, 202, 19, { align: 'right' })
                .text(config.website, 202, 31, { align: 'right' });
        ...
        doc.setLineWidth(0.8)
            ...
            .setFontSize(13)
                .setFont('FiraSansExtraLight').setTextColor(colors.dark)
                    .text('{{ trans_choice("position", 1) }}', 15, 63)
                    .text(billedPerProject ? '' : '{{ __("quantity") }}', 136, 63, { align: 'center' })
                    .text(billedPerProject ? '' : '{{ __("price") }}', 162, 63, { align: 'center' })
                .setFont('FiraSansRegular').setTextColor(colors.light)
                    .text(billedPerProject ? '' : '{{ __("total") }}', 189, 63, { align: 'center' })
            .setFontSize(8)
                .setFont('FiraSansExtraLight').setTextColor(colors.dark)
                    .text(undated ? '{{ __("description") }}' : '{{ __("dateAndDescription") }}', 15, 69)
                    .text(billedPerProject ? '' : '{{ __("inHours") }}', 136, 69, { align: 'center' })
                    .text(billedPerProject ? '' : '{{ __("perHour") }}', 162, 69, { align: 'center' })
                .setTextColor(colors.light)
                    .text(billedPerProject ? '' : '{{ __("price") }}', 189, 69, { align: 'center' });
        // draw positions
        let linesProcessed = 0;
        positions.forEach((p, i) => {
            const posdate = hdate(new Date(p.started_at), '{{ $this->record->project->client->language }}');
            ...
            doc.setTextColor(colors.dark)
                .setFont('FiraSansRegular')
                    .setFontSize(9)
                        .text(
                            undated ? (i+1).toString() + '. {{ trans_choice("position", 1) }}' : posdate,
                            15,
                ...
                .setFont('FiraSansExtraLight')
                    ...
                    .setFontSize(11)
                        .text(
                            billedPerProject ? '' : nDigit(poshours, 1, '{{ $this->record->project->client->language }}'),
                            136,
                            (87+positionRowHeight*linesProcessed), { align: 'center' }
                        ).text(
                            billedPerProject ? '' : euro({{ $this->record->price }}, '{{ $this->record->project->client->language }}'),
                            162,
                            (87+positionRowHeight*linesProcessed), { align: 'center' }
                        )
                    .setTextColor(colors.light)
                        .text(
                            billedPerProject ? '' : euro(poshours*{{ $this->record->price }}, '{{ $this->record->project->client->language }}'),
                            189,
                            (87+positionRowHeight*linesProcessed), { align: 'center' }
                        );
            linesProcessed += lineCount;
